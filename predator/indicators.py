...
def adx(highs: Any, lows: Any, closes: Any, period: int = 14) -> dict:
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        pdi = np.where(str_ != 0, 100 * spdm / str_, 0)
        mdi = np.where(str_ != 0, 100 * smdm / str_, 0)
        dx = np.where((pdi + mdi) != 0, 100 * np.abs(pdi - mdi) / (pdi + mdi), 0)
    if np.isnan(dx[-period]).any() if dx.size else True:
        adx_val = float(dx[period:].mean()) if dx.size > period else 0.0
    else:
