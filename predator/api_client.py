...
def _ideal_cmd(cmd: str, params: dict | None = None, timeout: int = 12,
    ...
    try:
        r = _SESSION.get(url, headers=_HEADERS, timeout=timeout, verify=False)
...
def fetch_burgan_kart(code: str) -> Any:
    ...
    try:
        r = _SESSION.get(
            config.BURGAN_API_URL,
            params={"hisseKodu": code},
            headers=_HEADERS,
            timeout=10,
            verify=False,
        )
...
def fetch_bist_full_list() -> list[dict]:
    ...
    for attempt in range(3):
        ...
        try:
            r = _SESSION.get(config.BURGAN_API_URL, headers=headers,
                             timeout=timeouts[attempt], verify=False)
